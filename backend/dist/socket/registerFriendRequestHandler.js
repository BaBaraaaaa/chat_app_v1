...
exports.registerFriendRequestHandler = void 0;
const friendService_1 = require("../services/friendService");
const registerFriendRequestHandler = (io, socket, onlineUsers) => {
    // 📨 Gửi lời mời kết bạn
    ...
    socket.on("RESPOND_FRIEND_REQUEST", async (data) => {
        try {
            ...
            if (response === "accepted") {
                result = await friendService_1.FriendService.acceptFriendRequest(requestId, userId);
            }
            else {
                result = await friendService_1.FriendService.declineFriendRequest(requestId, userId);
            }
            if (result.success) {
                // 📤 Thông báo cho người gửi nếu họ đang online
                const fromUserId = result.data?.fromUserId?._id || result.data?.fromUserId;
                const sender = onlineUsers.find((u) => u.userId === fromUserId?.toString());
                if (sender) {
                ...
                console.log(`✅ ${userId} ${response === "accepted" ? "chấp nhận" : "từ chối"} lời mời của ${fromUserId}`);
            }
            else {
                socket.emit("RESPOND_FRIEND_REQUEST_ERROR", {
                    success: false,
    ...
    socket.on("CANCEL_FRIEND_REQUEST", async (data) => {
        try {
            ...
            const result = await friendService_1.FriendService.cancelFriendRequest(requestId, userId);
            if (result.success) {
                // 📤 Thông báo cho người nhận nếu họ đang online  
                const toUserId = result.data?.toUserId?._id || result.data?.toUserId;
                const recipient = onlineUsers.find((u) => u.userId === toUserId?.toString());
                if (recipient) {
    ...
    socket.on("GET_FRIEND_REQUESTS", async () => {
        try {
            ...
            const result = await friendService_1.FriendService.getFriendRequests(userId);
            socket.emit("FRIEND_REQUESTS_LIST", {
    ...
    socket.on("GET_FRIENDS_LIST", async () => {
        try {
            ...
            const result = await friendService_1.FriendService.getFriendsList(userId);
            socket.emit("FRIENDS_LIST", {
                success: result.success,
                message: result.message,
                data: result.data
    ...
    socket.on("REMOVE_FRIEND", async (data) => {
        try {
            ...
            const result = await friendService_1.FriendService.removeFriend(userId, friendId);
            if (result.success) {
                // 📤 Thông báo cho người bạn bị xóa nếu họ đang online
                const removedFriend = onlineUsers.find((u) => u.userId === friendId.toString());
                if (removedFriend) {
...
exports.registerFriendRequestHandler = registerFriendRequestHandler;
//# sourceMappingURL=registerFriendRequestHandler.js.map