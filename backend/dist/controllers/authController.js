...
const signUp = async (req, res) => {
    try {
        ...
        //mã hóa mật khẩu
        const hashedPassword = await bcrypt_1.default.hash(password, 10); // 10 là số vòng băm
        //tạo user mới
...
const signIn = async (req, res) => {
    try {
        ...
        const accessToken = jsonwebtoken_1.default.sign({ userId: user._id }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: ACCESS_TOKEN_TTL });
        //Tạo refresh token
...
const refreshToken = async (req, res) => {
    try {
        ...
        const session = await Session_1.default.findOne({ refreshToken: token });
        if (!session) {
