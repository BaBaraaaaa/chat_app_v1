...
class MessageService {
   ...
   * Lấy danh sách tin nhắn áp dụng limit-offset Pagination
   */
  getMessagesByCursor(payload: GetMessagesByCursorPayload): void {
    if (socketService.isConnected()) {
        console.log(payload);
      socketService.emit("GET_MESSAGES_BY_CURSOR", payload);
    } else {
      toast.warning("⚠️ Socket chưa kết nối, không thể lấy tin nhắn");
    }
  }
   ...
   */
  onMarkReadSuccess(
    callback: SocketEventCallback<MarkReadSuccessResponse>
  ): void {
    socketService.on(
      "MARK_READ_SUCCESS",
  ...
  onMarkAllReadSuccess(
    callback: SocketEventCallback<MarkAllReadSuccessResponse>
  ): void {
    socketService.on(
  ...
  onDeleteMessageSuccess(
    callback: SocketEventCallback<DeleteMessageSuccessResponse>
  ): void {
    socketService.on(
   ...
   */
  onMessageDeleted(
    callback: SocketEventCallback<MessageDeletedResponse>
  ): void {
    socketService.on(
      "MESSAGE_DELETED",
      callback as (...args: unknown[]) => void
  ...
  onMessageNotification(
    callback: SocketEventCallback<MessageNotificationData>
  ): void {
    socketService.on(
   ...
   */
  onMarkAllReadError(
    callback: SocketEventCallback<MessageErrorResponse>
  ): void {
    socketService.on(
      "MARK_ALL_READ_ERROR",
      callback as (...args: unknown[]) => void
